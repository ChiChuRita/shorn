...
These also keep structure outside the payload, so the comparison is about where the schema lives. Theirs is a separate artifact to keep in sync — a `.proto` file, an Avro schema, a builder DSL. shorn's is the validator you already run: real code your compiler checks and your editor refactors. Types come from the validator's own inference, a single type lookup, so shorn adds no type-level work for tsserver.
